...
bool		Range::initialize()
{
  if (_texture.load("./img/range.tga") == false)
    {
      std::cerr << "Cannot load the range texture" << std::endl;
      return (false);
    }
...
bool		Range::update(gdl::SdlContext context, gdl::Input &input)
{
  (void)context;
  (void)input;
  return true;
...
void		Range::draw(gdl::AShader &shader, int x, int y)
{
...
glm::mat4	Range::getTransformation()
{
