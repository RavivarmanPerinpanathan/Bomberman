...
void	Player::update(gdl::Clock const &clock, gdl::Input &input)
{
  ...
  (void)clock;
  (void)input;
  if (getId() == 1 && input.getKey(SDLK_UP))
    updateMap(getPos(), std::make_pair(-1, 0));
  if (getId() == 1 && input.getKey(SDLK_DOWN))
    updateMap(getPos(), std::make_pair(1, 0));
  if (getId() == 1 && input.getKey(SDLK_LEFT))
    updateMap(getPos(), std::make_pair(0, -1));
  if (getId() == 1 && input.getKey(SDLK_RIGHT))
    updateMap(getPos(), std::make_pair(0, 1));
  if (getId() == 0 && input.getKey(SDLK_z))
    updateMap(getPos(), std::make_pair(-1, 0));
  if (getId() == 0 && input.getKey(SDLK_s))
    updateMap(getPos(), std::make_pair(1, 0));
  if (getId() == 0 && input.getKey(SDLK_q))
    updateMap(getPos(), std::make_pair(0, -1));
  if (getId() == 0 && input.getKey(SDLK_d))
    updateMap(getPos(), std::make_pair(0, 1));
}
